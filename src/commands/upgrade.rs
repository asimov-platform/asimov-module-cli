...
pub async fn upgrade(
    ...
    for module_name in module_names {
        let installed = installed_version(&module_name).await.inspect_err(|_| {
            ceprintln!("<s,r>error:</> failed to read installed version of `{module_name}`")
        })?;
        ...
        install_module_manifest(&module_name, &latest)
            .await
            .inspect_err(|_| {
                ceprintln!("<s,r>error:</> failed to install module manifest for `{module_name}`")
            })?;
